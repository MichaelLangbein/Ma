...
Promise: 
 ...
 - however, data is delivered in a single batch - there is no \inlinecode{observer.onCompleted()} method
observable: 
 - vs promises: 
        - observables are like promises, but they can emmit multiple calls to \inlinecode{observer.onNext(data)}, and are cancelable and unsubscribeable.
 - vs subjects: 
...
subject: 
 ...
 - \inlinecode{next} is called from outside the subject, instead of from inside like it is the case with observables. This makes that basically anyone can trigger a broadcast. 
behaviour subjects: 
