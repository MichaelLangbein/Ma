...
\paragraph{Dirty-checking}: This is how angular does change detection:
\begin{lstlisting}
// dirtychecking: this is inside `checkAndUpdateView`
for (const view in views) {
  for (const binding in view.bindings) {
    const newVal = binding.expression()
    const oldVal = view.oldValues[i]
    if (newVal != oldVal) {
      updateDomDownFrom(view)
    }
  }
}
\end{lstlisting}
