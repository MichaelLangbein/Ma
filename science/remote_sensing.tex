...
\paragraph{Polarization}: EM waves have a plane. In fact, there is a plane for the electrical wave and a perpendicular one for the magnetic field.
...
\paragraph{Modis on Aqua and Terra} (NASA) is a spectro-radiometer. It measures 36 bands.
...
Where STAC/COG data is not available, you'll have to fall back to proprietary solutions. One nice library is ESA's \inlinecode{sentinelsat}.
...
\subsection{Remote sensing for floods}
...
\end{lstlisting}
Note that the \inlinecode{sourcefile.xml} will overwrite the \inlinecode{read/parameter/file} value in the graph-file.
There is a nice tutorial on headless-SNAP \href{http://step.esa.int/docs/tutorials/SNAP_CommandLine_Tutorial.pdf}{here}.

Especially convenient is the fact that one can  insert variables in the form \inlinecode{\$\{variableName\}} in place
of a parameter value. You can then replace the variableName with a value at the command line.
For example, if a parameter for a file included the variable for \inlinecode{\$\{myFilename\}}
\begin{lstlisting}
    <parameters>
     <file>${myFilename}</file>
    </parameters>
\end{lstlisting}
and then call gpt using \inlinecode{gpt mygraph.xml -PmyFilename=path/to/my/file}.
