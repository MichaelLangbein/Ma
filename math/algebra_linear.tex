...
A common application of SVA is PCA.
...
Thus the eigenvector-matrix of $Cov_A$ is $E_{Cov_A} = E_{A^T A} = V$ and writing out the full eigenvector-formula we get 
$$ Cov_A E_{Cov_A} = E_{Cov_A} \Lambda_{Cov_A} $$

Define the principal component matrix $P$ as 
\begin{equation}
    \begin{aligned}
         P  &= X E_{Cov_A} \\
...
We can then use the first few principal components to get a good approximation of the complete dataset $A$.
