...
Consider some basis $B_{\rowspace{A}}$. Through the transformation $\mtrx{A}: \reals^n \then \reals^m$ that basis $B_{\rowspace{A}}$ is transformed into \collspace{A}.
So far so easy - but we have an additional requirement: we want that transformed basis to be a basis itself: $ \mtrx{A} B_{\rowspace{A}} : B_{\collspace{A}}$.

There may be many bases $B_{\rowspace{A}}$, but with that additional requirement, it must truly be a special basis. Let's call it $\mtrx{V}$, and the resulting new basis $\mtrx{A} \mtrx{V} = \mtrx{U}$.
...
Summarizing our requirements:
\begin{itemize}
    \item V is a basis for the rowspace: $\mtrx{V}: B_{\rowspace{A}}$
    \item The transformation of V, $\mtrx{A} \mtrx{V} = \mtrx{U}$, is a basis for the collumnspace $\mtrx{U}: B_{\collspace{A}}$
    \item The normalizers of U are called $\mtrx{\Sigma}$
\end{itemize}
...
Our goal is to find a matrix $V$ such that $V: B_{\rowspace{A}}$ \emph{and also} $AV = U': B_{\collspace{A}}$.
...
$$ B = E_{Cov_A}^T A $$
\footnote{
    Compare section \ref{changeOfBasis}: 
    \begin{equation}
        \begin{aligned}
            (\vec{v})_B &= \mtrx{B}^{-1} \mtrx{A} (\vec{v})_A \\
                        &= \mtrx{B}^{-1} \mtrx{I} (\vec{v})_A \\
                        &= \mtrx{E}^{-1} (\vec{v})_A \\
                        &= \mtrx{E}^T (\vec{v})_A
        \end{aligned}
    \end{equation}
