...
\end{bmatrix}$. Now consider $\vec{v} = \begin{bmatrix}
    1 \\ 1
\end{bmatrix}$, which is neither in \nullspace{A} nor in \collspace{A^T}.
...
There may be many bases $B_{\collspace{A^T}}$, but with that additional requirement, it must truely be a spetial basis. Let's call it $\mtrx{V}$, and the resulting new basis $\mtrx{A} \mtrx{V} = \mtrx{U}$.
