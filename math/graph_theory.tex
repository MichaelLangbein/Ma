...
\begin{proof} In a binary tree, the number of leaves $n$ equals $\frac{N+1}{2}$, regardless of the architecture of the tree.
    ...
    \subprf{}{$n = \frac{N+1}{2}$}{
        ...
        Now consider the nodes (there are $N-n$ of them). They all have $\outdeg(v_n) = 2$ and $\intdeg(v_n) = 1$, except for the root-node. Thus $\sum_{\text{nodes}} \indeg(v_n) = N - n - 1$ and $\sum_{\text{nodes}} \outdeg(v_n) = 2(N - n)$. \\
